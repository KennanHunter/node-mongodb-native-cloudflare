    ...
    async _connect() {
        ...
        if (options.tls) {
            if (typeof options.tlsCAFile === 'string') {
                options.ca ??= await node_fs_1.promises.readFile(options.tlsCAFile);
            }
            if (typeof options.tlsCRLFile === 'string') {
                options.crl ??= await node_fs_1.promises.readFile(options.tlsCRLFile);
            }
            if (typeof options.tlsCertificateKeyFile === 'string') {
                if (!options.key || !options.cert) {
                    const contents = await node_fs_1.promises.readFile(options.tlsCertificateKeyFile);
